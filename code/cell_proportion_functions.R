...
getCellRatioData <- function(data, dataname, cancer){
  data.long <- data %>%
    as.data.frame() %>%
    rownames_to_column("Cell Type") %>%
    pivot_longer(cols = -`Cell Type`, names_to = "Sample", values_to = "Proportion") %>%
    mutate(Source=dataname)
  
  if (!hasArg("unite")) {
    Update = T
    
    celltype.mapping = read_excel("../01.data/cell_type_summary.xlsx")
    data.long = data.long %>%
      mutate(`Cell Type`=celltype.mapping$cellType.unite[match(`Cell Type`, celltype.mapping$cellType.origin)])
  }
...
checkNormalDistribution <- function(data, selected_cell.type, dataname){
  
  png(filename =paste0("../04.plot/", dataname, ".tumor.ratio.distribution.png"), width = 1400, height = 450,
      pointsize = 12, bg = "white")
  
  data.list <- data %>%
    filter(`Cell Type` %in% selected_cell.type) %>% select(Proportion) %>% pull()
  
  # plot layout
  combined_plot <- par(mfrow=c(1, 1))
  layout(matrix(c(1, 2), nrow=1))
  
  hist(data.list, sub=paste(selected_cell.type, collapse = " "))
  qqnorm(data.list)
  qqline(data.list)
  
  par(combined_plot)
  
  title(main=print(paste0(dataname, " tumor cell propotyion distribution")),
